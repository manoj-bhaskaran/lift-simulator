...
Current version: **0.57.2**. This project follows [Semantic Versioning](https://semver.org/); see [CHANGELOG.md](CHANGELOG.md) for version history.
...
java -jar target/lift-simulator-0.57.2.jar
```
...
```bash
jar tf target/lift-simulator-0.57.2.jar | grep '^BOOT-INF/classes/static/'
```
...
```bash
java -cp target/lift-simulator-0.57.2.jar com.liftsimulator.Main --help
java -cp target/lift-simulator-0.57.2.jar com.liftsimulator.Main --strategy=directional-scan
```
...
java -cp target/lift-simulator-0.57.2.jar com.liftsimulator.scenario.ScenarioRunnerMain path/to/scenario.scenario
```
...
- `application.yml` — base defaults (application name `lift-config-service`, server port `8080`, actuator `health`/`info`, root log level `INFO` and `DEBUG` for `com.liftsimulator`, and file output at `logs/application.log`)
