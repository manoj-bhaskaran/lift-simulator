...
Current version: **0.52.2**. This project follows [Semantic Versioning](https://semver.org/); see [CHANGELOG.md](CHANGELOG.md) for version history.
...
java -jar target/lift-simulator-0.52.2.jar
```
...
```bash
jar tf target/lift-simulator-0.52.2.jar | grep '^BOOT-INF/classes/static/'
```
...
```bash
java -cp target/lift-simulator-0.52.2.jar com.liftsimulator.Main --help
java -cp target/lift-simulator-0.52.2.jar com.liftsimulator.Main --strategy=directional-scan
```
...
```bash
java -cp target/lift-simulator-0.52.2.jar com.liftsimulator.runtime.LocalSimulationMain --config=path/to/config.json
```
...
java -cp target/lift-simulator-0.52.2.jar com.liftsimulator.scenario.ScenarioRunnerMain path/to/scenario.scenario
```
