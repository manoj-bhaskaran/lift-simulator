...
The current version (v0.12.8) implements:
...
The packaged JAR will be in `target/lift-simulator-0.12.8.jar`.
...
```bash
java -cp target/lift-simulator-0.12.8.jar com.liftsimulator.Main
```
...
```bash
java -cp target/lift-simulator-0.12.8.jar com.liftsimulator.scenario.ScenarioRunnerMain path/to/scenario.scenario
```
