...
Current version: **0.53.9**. This project follows [Semantic Versioning](https://semver.org/); see [CHANGELOG.md](CHANGELOG.md) for version history.
...
java -jar target/lift-simulator-0.53.9.jar
```
...
```bash
jar tf target/lift-simulator-0.53.9.jar | grep '^BOOT-INF/classes/static/'
```
...
```bash
java -cp target/lift-simulator-0.53.9.jar com.liftsimulator.Main --help
java -cp target/lift-simulator-0.53.9.jar com.liftsimulator.Main --strategy=directional-scan
```
...
java -cp target/lift-simulator-0.53.9.jar com.liftsimulator.scenario.ScenarioRunnerMain path/to/scenario.scenario
```
...
**Backend dependency baseline:** Spring Boot 3.5.15, Springdoc OpenAPI 2.7.0, PostgreSQL JDBC driver 42.7.11, and the Flyway PostgreSQL module (managed by Spring Boot dependency management). The temporary `spring-boot-properties-migrator` runtime dependency is enabled during the 3.5.x transition to report configuration keys that need migration before the Spring Boot 4 upgrade.
